...
#include "pr_dstrings.h"
#include "pr_sounds.h"
#include "pr_r_data.h"
#include "pr_r_sky.h"
#include "pr_d_deh.h"              // Ty 3/27/98 deh declarations
...
namespace prboom
{
char PACKAGE_VERSION[128]; // no idea where this actually defined..
...
{
  special_event = (buttoncode_t)(BT_SPECIAL | (BTS_RESTARTLEVEL & BT_SPECIALMASK));
}
...
static void G_DoLoadLevel (void)
{
  ...
  mousex = mousey = 0;
  mlooky = 0;//e6y
  special_event = (buttoncode_t)0; paused = false;
  ...
  switch (ev->type)
    ...
    case ev_keydown:
      if (ev->data1 == key_pause)           // phares
        {
          special_event = (buttoncode_t)(BT_SPECIAL | (BTS_PAUSE & BT_SPECIALMASK));
          return true;
        }
      if (ev->data1 <NUMKEYS)
...
static uint_64_t G_UpdateSignature(uint_64_t s, const char *name)
{
  ...
  const byte *p = (const byte *)W_CacheLumpNum(i);
  while (size--)
    s <<= 1, s += *p++;
  W_UnlockLumpNum(i);
      }
...
void G_LoadGame(int slot, dboolean command)
{
  if (!demoplayback && !command) {
    ...
    special_event = (buttoncode_t)(BT_SPECIAL | (BTS_LOADGAME & BT_SPECIALMASK) |
...
void G_DoLoadGame(void)
{
  ...
  {  // killough 3/16/98: check lump name checksum (independent of order)
    ...
    if (memcmp(&checksum, save_p, sizeof checksum)) {
      if (!forced_loadgame) {
        char *msg = (char*)malloc(strlen((char*)save_p + sizeof checksum) + 128);
...
void G_SaveGame(int slot, char *description)
{
  ...
  special_event = (buttoncode_t)(BT_SPECIAL | (BTS_SAVEGAME & BT_SPECIALMASK) |
    ((slot << BTS_SAVESHIFT) & BTS_SAVEMASK));
#ifdef HAVE_NET
...
void G_RecordDemo (const char* name)
{
  char *demoname;
  usergame = false;
  demoname = (char*)malloc(strlen(name)+4+1);
...
void G_BeginRecording (void)
{
  ...
  demostart = demo_p = (byte*)malloc(1000);
  longtics = 0;
