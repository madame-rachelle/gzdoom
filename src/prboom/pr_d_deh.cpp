...
#include "pr_doomdef.h"
#include "pr_doomtype.h"
#include "pr_doomstat.h"
#include "pr_d_deh.h"
#include "pr_sounds.h"
#include "pr_info.h"
#include "pr_m_cheat.h"
#include "pr_p_inter.h"
#include "pr_p_enemy.h"
#include "pr_g_game.h"
#include "pr_d_think.h"
#include "pr_w_wad.h"
#include "pr_m_argv.h"
#include "pr_m_misc.h"
#include "pr_e6y.h"//e6y

// CPhipps - modify to use logical output routine
#include "pr_lprintf.h"

#include "pr_dstrings.h"  // to get the initial values

#define TRUE 1
#define FALSE 0

#define PACKAGE_TARNAME "bsboom"
...
{
  {(actionf_t)A_Light0,          "A_Light0"},
  {(actionf_t)A_WeaponReady,     "A_WeaponReady"},
  {(actionf_t)A_Lower,           "A_Lower"},
  {(actionf_t)A_Raise,           "A_Raise"},
  {(actionf_t)A_Punch,           "A_Punch"},
  {(actionf_t)A_ReFire,          "A_ReFire"},
  {(actionf_t)A_FirePistol,      "A_FirePistol"},
  {(actionf_t)A_Light1,          "A_Light1"},
  {(actionf_t)A_FireShotgun,     "A_FireShotgun"},
  {(actionf_t)A_Light2,          "A_Light2"},
  {(actionf_t)A_FireShotgun2,    "A_FireShotgun2"},
  {(actionf_t)A_CheckReload,     "A_CheckReload"},
  {(actionf_t)A_OpenShotgun2,    "A_OpenShotgun2"},
  {(actionf_t)A_LoadShotgun2,    "A_LoadShotgun2"},
  {(actionf_t)A_CloseShotgun2,   "A_CloseShotgun2"},
  {(actionf_t)A_FireCGun,        "A_FireCGun"},
  {(actionf_t)A_GunFlash,        "A_GunFlash"},
  {(actionf_t)A_FireMissile,     "A_FireMissile"},
  {(actionf_t)A_Saw,             "A_Saw"},
  {(actionf_t)A_FirePlasma,      "A_FirePlasma"},
  {(actionf_t)A_BFGsound,        "A_BFGsound"},
  {(actionf_t)A_FireBFG,         "A_FireBFG"},
  {(actionf_t)A_BFGSpray,        "A_BFGSpray"},
  {(actionf_t)A_Explode,         "A_Explode"},
  {(actionf_t)A_Pain,            "A_Pain"},
  {(actionf_t)A_PlayerScream,    "A_PlayerScream"},
  {(actionf_t)A_Fall,            "A_Fall"},
  {(actionf_t)A_XScream,         "A_XScream"},
  {(actionf_t)A_Look,            "A_Look"},
  {(actionf_t)A_Chase,           "A_Chase"},
  {(actionf_t)A_FaceTarget,      "A_FaceTarget"},
  {(actionf_t)A_PosAttack,       "A_PosAttack"},
  {(actionf_t)A_Scream,          "A_Scream"},
  {(actionf_t)A_SPosAttack,      "A_SPosAttack"},
  {(actionf_t)A_VileChase,       "A_VileChase"},
  {(actionf_t)A_VileStart,       "A_VileStart"},
  {(actionf_t)A_VileTarget,      "A_VileTarget"},
  {(actionf_t)A_VileAttack,      "A_VileAttack"},
  {(actionf_t)A_StartFire,       "A_StartFire"},
  {(actionf_t)A_Fire,            "A_Fire"},
  {(actionf_t)A_FireCrackle,     "A_FireCrackle"},
  {(actionf_t)A_Tracer,          "A_Tracer"},
  {(actionf_t)A_SkelWhoosh,      "A_SkelWhoosh"},
  {(actionf_t)A_SkelFist,        "A_SkelFist"},
  {(actionf_t)A_SkelMissile,     "A_SkelMissile"},
  {(actionf_t)A_FatRaise,        "A_FatRaise"},
  {(actionf_t)A_FatAttack1,      "A_FatAttack1"},
  {(actionf_t)A_FatAttack2,      "A_FatAttack2"},
  {(actionf_t)A_FatAttack3,      "A_FatAttack3"},
  {(actionf_t)A_BossDeath,       "A_BossDeath"},
  {(actionf_t)A_CPosAttack,      "A_CPosAttack"},
  {(actionf_t)A_CPosRefire,      "A_CPosRefire"},
  {(actionf_t)A_TroopAttack,     "A_TroopAttack"},
  {(actionf_t)A_SargAttack,      "A_SargAttack"},
  {(actionf_t)A_HeadAttack,      "A_HeadAttack"},
  {(actionf_t)A_BruisAttack,     "A_BruisAttack"},
  {(actionf_t)A_SkullAttack,     "A_SkullAttack"},
  {(actionf_t)A_Metal,           "A_Metal"},
  {(actionf_t)A_SpidRefire,      "A_SpidRefire"},
  {(actionf_t)A_BabyMetal,       "A_BabyMetal"},
  {(actionf_t)A_BspiAttack,      "A_BspiAttack"},
  {(actionf_t)A_Hoof,            "A_Hoof"},
  {(actionf_t)A_CyberAttack,     "A_CyberAttack"},
  {(actionf_t)A_PainAttack,      "A_PainAttack"},
  {(actionf_t)A_PainDie,         "A_PainDie"},
  {(actionf_t)A_KeenDie,         "A_KeenDie"},
  {(actionf_t)A_BrainPain,       "A_BrainPain"},
  {(actionf_t)A_BrainScream,     "A_BrainScream"},
  {(actionf_t)A_BrainDie,        "A_BrainDie"},
  {(actionf_t)A_BrainAwake,      "A_BrainAwake"},
  {(actionf_t)A_BrainSpit,       "A_BrainSpit"},
  {(actionf_t)A_SpawnSound,      "A_SpawnSound"},
  {(actionf_t)A_SpawnFly,        "A_SpawnFly"},
  {(actionf_t)A_BrainExplode,    "A_BrainExplode"},
  {(actionf_t)A_Detonate,        "A_Detonate"},       // killough 8/9/98
  {(actionf_t)A_Mushroom,        "A_Mushroom"},       // killough 10/98
  {(actionf_t)A_Die,             "A_Die"},            // killough 11/98
  {(actionf_t)A_Spawn,           "A_Spawn"},          // killough 11/98
  {(actionf_t)A_Turn,            "A_Turn"},           // killough 11/98
  {(actionf_t)A_Face,            "A_Face"},           // killough 11/98
  {(actionf_t)A_Scratch,         "A_Scratch"},        // killough 11/98
  {(actionf_t)A_PlaySound,       "A_PlaySound"},      // killough 11/98
  {(actionf_t)A_RandomJump,      "A_RandomJump"},     // killough 11/98
  {(actionf_t)A_LineEffect,      "A_LineEffect"},     // killough 11/98

  {(actionf_t)A_FireOldBFG,      "A_FireOldBFG"},      // killough 7/19/98: classic BFG firing function
  {(actionf_t)A_BetaSkullAttack, "A_BetaSkullAttack"}, // killough 10/98: beta lost souls attacked different
  {(actionf_t)A_Stop,            "A_Stop"},
  ...
  while (!dehfeof(fpin) && *inbuffer)  /* && (*inbuffer != ' ') */
    ...
    // killough 11/98: allocate enough the first time
          ...
          if (fpout) fprintf(fpout,
                             ...
                             (int)maxstrlen,(int)strlen(inbuffer));
