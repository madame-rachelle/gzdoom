...
#include "pr_r_main.h"
#include "pr_p_maputl.h"
#include "pr_p_spec.h"
#include "pr_p_tick.h"
#include "pr_p_saveg.h"
#include "pr_m_random.h"
#include "pr_am_map.h"
  ...
  for (i=0, sec = sectors ; i<numsectors ; i++,sec++)
    {
      ...
      put = (short*)(void *)((char *) put + sizeof sec->floorheight);
      memcpy(put, &sec->ceilingheight, sizeof sec->ceilingheight);
      put = (short*)(void *)((char *) put + sizeof sec->ceilingheight);
  ...
  for (i=0, li = lines ; i<numlines ; i++,li++)
    {
      ...
      *put++ = li->tag;
      ...
      put = (short*)(void *)((char *) put + sizeof si->textureoffset);
      memcpy(put, &si->rowoffset, sizeof si->rowoffset);
      put = (short*)(void *)((char *) put + sizeof si->rowoffset);
  ...
  for (i=0, sec = sectors ; i<numsectors ; i++,sec++)
    {
      ...
      get = (short *)(void *)((char *) get + sizeof sec->floorheight);
      memcpy(&sec->ceilingheight, get, sizeof sec->ceilingheight);
      get = (short *)(void *)((char *) get + sizeof sec->ceilingheight);
  ...
  for (i=0, li = lines ; i<numlines ; i++,li++)
    {
      ...
      get = (short *)(void *)((char *) get + sizeof si->textureoffset);
      memcpy(&si->rowoffset, get, sizeof si->rowoffset);
      get = (short *)(void *)((char *) get + sizeof si->rowoffset);
  ...
  *save_p++ = tc_end;
  ...
  // killough 9/14/98: save soundtargets
  {
    ...
    for (i = 0; i < numsectors; i++)
    {
      ...
      // (prboom bug #1590350)
      if (target && target->thinker.function == (think_t)P_MobjThinker)
...
void P_ArchiveSpecials (void)
{
  ...
  for (th = thinkercap.next ; th != &thinkercap ; th=th->next)
    if (!th->function)
      ...
      size +=
        th->function==(think_t)T_MoveCeiling  ? 4+sizeof(ceiling_t) :
        th->function==(think_t)T_VerticalDoor ? 4+sizeof(vldoor_t)  :
        th->function==(think_t)T_MoveFloor    ? 4+sizeof(floormove_t):
        th->function==(think_t)T_PlatRaise    ? 4+sizeof(plat_t)    :
        th->function==(think_t)T_LightFlash   ? 4+sizeof(lightflash_t):
        th->function==(think_t)T_StrobeFlash  ? 4+sizeof(strobe_t)  :
        th->function==(think_t)T_Glow         ? 4+sizeof(glow_t)    :
        th->function==(think_t)T_MoveElevator ? 4+sizeof(elevator_t):
        th->function==(think_t)T_Scroll       ? 4+sizeof(scroll_t)  :
        th->function==(think_t)T_Pusher       ? 4+sizeof(pusher_t)  :
        th->function==(think_t)T_FireFlicker? 4+sizeof(fireflicker_t) :
      0;
  ...
  for (th=thinkercap.next; th!=&thinkercap; th=th->next)
    {
      ...
      if (th->function == (think_t)T_FireFlicker)
        {
      ...
      if (th->function == (think_t)T_Pusher)
        {
          *save_p++ = tc_pusher;
  ...
  while ((tclass = *save_p++) != tc_endspecials)  // killough 2/14/98
    switch (tclass)
      ...
      case tc_floor:
        PADSAVEP();
        {
          floormove_t *floor = (floormove_t*)Z_Malloc (sizeof(*floor), PU_LEVEL, NULL);
      ...
      case tc_flash:
        PADSAVEP();
        {
          lightflash_t *flash = (lightflash_t*)Z_Malloc (sizeof(*flash), PU_LEVEL, NULL);
      ...
      case tc_strobe:
        PADSAVEP();
        {
          strobe_t *strobe = (strobe_t*)Z_Malloc (sizeof(*strobe), PU_LEVEL, NULL);
      ...
      case tc_flicker:           // killough 10/4/98
        PADSAVEP();
        {
          fireflicker_t *flicker = (fireflicker_t*)Z_Malloc (sizeof(*flicker), PU_LEVEL, NULL);
      ...
      case tc_elevator:
        PADSAVEP();
        {
          elevator_t *elevator = (elevator_t*)Z_Malloc (sizeof(*elevator), PU_LEVEL, NULL);
      ...
      case tc_scroll:       // killough 3/7/98: scroll effect thinkers
        {
          scroll_t *scroll = (scroll_t*)Z_Malloc (sizeof(scroll_t), PU_LEVEL, NULL);
          memcpy (scroll, save_p, sizeof(scroll_t));
          save_p += sizeof(scroll_t);
          scroll->thinker.function = (think_t)T_Scroll;
      ...
      case tc_pusher:   // phares 3/22/98: new Push/Pull effect thinkers
        {
          pusher_t *pusher = (pusher_t*)Z_Malloc (sizeof(pusher_t), PU_LEVEL, NULL);
          memcpy (pusher, save_p, sizeof(pusher_t));
          save_p += sizeof(pusher_t);
          pusher->thinker.function = (think_t)T_Pusher;
  ...
  if (markpointnum)
    {
      ...
      while (markpointnum >= markpointnum_max)
        markpoints = (markpoint_t*)realloc(markpoints, sizeof *markpoints *
