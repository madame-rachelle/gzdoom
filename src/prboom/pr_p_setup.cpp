...
#include "pr_p_map.h"
#include "pr_p_setup.h"
#include "pr_p_spec.h"
#include "pr_p_tick.h"
#include "pr_p_enemy.h"
#include "pr_s_sound.h"
#include "pr_s_advsound.h"
#include "pr_lprintf.h" //jff 10/6/98 for debug outputs
#include "pr_v_video.h"
#include "pr_r_demo.h"
#include "pr_r_fps.h"
#include "pr_hu_tracers.h"
#include "pr_g_overflow.h"
#include "pr_am_map.h"
...
static void P_LoadVertexes (int lump)
{
  ...
  vertexes = (vertex_t*)calloc_IfSameLevel(vertexes, numvertexes, sizeof(vertex_t));
...
static void P_LoadVertexes2(int lump, int gllump)
{
  ...
  {
    gldata = (const byte*)W_CacheLumpNum(gllump);
    ...
    if (nodesVersion == gNd2) // 32 bit GL_VERT format (16.16 fixed)
    {
      ...
      vertexes = (vertex_t*)malloc_IfSameLevel(vertexes, numvertexes * sizeof(vertex_t));
...
static void P_LoadSegs (int lump)
{
  ...
  segs = (seg_t*)calloc_IfSameLevel(segs, numsegs, sizeof(seg_t));
...
static void P_LoadGLSegs(int lump)
{
  ...
  segs = (seg_t*)malloc_IfSameLevel(segs, numsegs * sizeof(seg_t));
...
static void P_LoadSubsectors (int lump)
{
  ...
  subsectors = (subsector_t*)calloc_IfSameLevel(subsectors, numsubsectors, sizeof(subsector_t));
...
static void P_LoadSectors (int lump)
{
  ...
  sectors = (sector_t*)calloc_IfSameLevel(sectors, numsectors, sizeof(sector_t));
...
static void P_LoadNodes (int lump)
{
  ...
  nodes = (node_t*)malloc_IfSameLevel(nodes, numnodes * sizeof(node_t));
  data = (const byte *)W_CacheLumpNum (lump); // cph - wad lump handling updated
...
static void P_LoadZNodes(int lump, int glnodes)
{
  ...
  numnodes = numNodes;
  nodes = (node_t*)calloc_IfSameLevel(nodes, numNodes, sizeof(node_t));
...
static void P_LoadThings (int lump)
{
  ...
  mobj_t *mobj;
  int mobjcount = 0;
  mobj_t **mobjlist = (mobj_t**)malloc(numthings * sizeof(mobjlist[0]));
...
static void P_LoadLineDefs (int lump)
{
  ...
  lines = (line_t*)calloc_IfSameLevel(lines, numlines, sizeof(line_t));
  data = (const byte *)W_CacheLumpNum (lump); // cph - wad lump handling updated
...
static void P_LoadSideDefs (int lump)
{
  ...
  sides = (side_t*)calloc_IfSameLevel(sides, numsides, sizeof(side_t));
}
...
static void P_CreateBlockMap(void)
{
  ...
  blockmaplump = (int*)malloc_IfSameLevel(blockmaplump, sizeof(*blockmaplump) * (4 + NBlocks + linetotal));
  // blockmap header
...
static void P_LoadBlockMap (int lump)
{
  ...
  if (M_CheckParm("-blockmap") || W_LumpLength(lump)<8 || (count = W_LumpLength(lump)/2) >= 0x10000) //e6y
  ...
  else
    {
      ...
      blockmaplump = (int*)malloc_IfSameLevel(blockmaplump, sizeof(*blockmaplump) * count);
  ...
  blocklinks = (mobj_t**)calloc_IfSameLevel(blocklinks, bmapwidth * bmapheight, sizeof(*blocklinks));
...
static void P_LoadReject(int lumpnum, int totallines)
{
  ...
  rejectmatrix = (const byte *)W_CacheLumpNum(rejectlump);
...
{
  fixed_t *bbox = (fixed_t*)(void*)sector->blockbox;
...
static int P_GroupLines (void)
{
  ...
  for (i=0, sector = sectors; i<numsectors; i++, sector++)
  {
    fixed_t *bbox = (fixed_t*)(void*)sector->blockbox; // cph - For convenience, so
...
void P_InitSubsectorsLines(void)
{
  ...
  count = 0;
  sslines_indexes = (int*)malloc((numsubsectors + 1) * sizeof(sslines_indexes[0]));
  ...
  sslines = (ssline_t*)malloc(count * sizeof(sslines[0]));
  count = 0;
