...
extern void I_PreventSleep(bool aye);
...
CUSTOM_CVAR(Bool, i_preventsystemsleep, true, CVAR_ARCHIVE|CVAR_GLOBALCONFIG|CVAR_NOINITCALL)
{
	I_PreventSleep(!!(self));
}
...
static int D_DoomMain_Internal (void)
{
	...
	do
	{
		...
		D_DoAnonStats();
		I_UpdateWindowTitle();
		I_PreventSleep(i_preventsystemsleep);
